...
public class UserService
{
    ...
    public ALLUserAttentionDTO getAllUserAttention(User userSession)
    {
        ...
        if (listProject.size() > 0)
        {
            for (AttentionProject attentionProject : listProject)
            {
                ...
                project = projectMapper.selectByPrimaryKey(
                    attentionProject.getAttentionProjectProject());
                if (project != null)
                {
                    listP.add(project);
                }
     ...
     * @return
     */
    public User updateBasic(String textName, String imgPath, User userSession, String imgPath2,
                            String sex, String your)
    {
     ...
     * @return
     */
    public String updateArticle(String article_Id, String titleName, String content,
                                String imgPath)
    {
        Article article = articleMapper.selectByPrimaryKey(article_Id);
     ...
     * @return
     */
    public String directlyReleaseArticle(User userSession, String titleName, String content,
                                         String imgPath)
    {
        Article article = new Article();
        if (titleName != null && titleName.trim().length() > 0)
     ...
     * @return
     */
    public String releaseArticle(String article_Id, String titleName, String content,
                                 String imgPath)
    {
        if (article_Id != null && article_Id.trim().length() > 0)
        {
    ...
    public List<User> findFansBy(User userSession)
    {
        if (userSession.getUserId() != null && userSession.getUserId().trim().length() > 0)
        {
            AttentionPeopleExample attentionPeopleExample = new AttentionPeopleExample();
            Criteria criteria = attentionPeopleExample.createCriteria();
            criteria.andAttentionPeopleUserTwoEqualTo(userSession.getUserId());
            List<AttentionPeople> listFansId = aPeopleMapper.selectByExample(
                attentionPeopleExample);
            List<User> listUser = new ArrayList<>();
            for (AttentionPeople attentionPeople : listFansId)
            {
                User user = new User();
                user = userMapper.selectByPrimaryKey(attentionPeople.getAttentionPeopleUserOne());
                if (user != null)
                {
                    listUser.add(user);
                }
